...
function ColumnRow({ tableName, col, referencedColumns, localFkColumns }: ColumnRowProps) {
    ...
    const isLocalFk = localFkColumns.includes(col.name);
    ...
    return (
        <div className={`relative flex items-center gap-3 px-4 py-2 hover:bg-white/[0.04] transition-colors ${col.primaryKey ? 'border-l-2 border-l-yellow-400/60 bg-yellow-400/[0.08]' : ''}`}>
            ...
            <div className="flex items-center gap-1 shrink-0 min-h-[20px]">
                {isLocalFk && <span className="px-1.5 py-0.5 rounded text-[10px] font-semibold bg-blue-500/20 text-blue-400">FK</span>}
